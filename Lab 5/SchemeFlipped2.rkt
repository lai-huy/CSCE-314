...
(define (count-nodes tree)
    (- (count-total tree) (count-leaves tree))
)

(define (count-total tree)
    (if (leaf? tree)
        1
        (count-total-in-forest (children tree))
    )
)

(define (count-total-in-forest forest)
    (if (null? forest)
        1
        (+ (count-total (car forest))
            (count-total-in-forest (cdr forest)))
  )
)
